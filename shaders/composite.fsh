...
vec4 positionInSunClipCoord = shadowProjection * positionInSunViewCoord;
 // 太阳的裁剪坐标 转 太阳的ndc坐标
vec4 positionInSunNdcCoord = 
vec4(positionInSunClipCoord.xyz/positionInSunClipCoord.w, 1.0);
 // 太阳的ndc坐标 转 太阳的屏幕坐标
...
const int shadowMapResolution = SHADOW_MAPRES;
if(closest <= currentDepth-0.0001) {
 color.rgb *= 0.5;   // 涂黑
}
 return color;
 }
...
vec4(positionInClipCoord.xyz/positionInClipCoord.w, 1.0);
